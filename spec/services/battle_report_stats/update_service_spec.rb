...
RSpec.describe BattleReportStats::UpdateService do
  ...
  let!(:company_stats1) { create :company_stats, company: company1,
                                 infantry_kills_2v2: prev_inf_killed_2v2, infantry_kills_total: prev_inf_killed_2v2,
                                 vehicle_losses_2v2: prev_veh_lost_2v2, vehicle_losses_total: prev_veh_lost_2v2 }
    ...
    context "when battle is final" do
      ...
      context "when the company is the winner" do
        ...
        it "updates infantry and vehicle kill loss" do
          ...
          expect(company_stats1.reload.infantry_kills_total).to eq inf_killed1 + prev_inf_killed_2v2
          expect(company_stats1.vehicle_kills_1v1).to eq veh_killed1
          expect(company_stats1.vehicle_kills_total).to eq veh_killed1
          expect(company_stats1.infantry_losses_1v1).to eq inf_lost1
          expect(company_stats1.infantry_losses_total).to eq inf_lost1
          expect(company_stats1.vehicle_losses_1v1).to eq veh_lost1
          expect(company_stats1.vehicle_losses_total).to eq veh_lost1 + prev_veh_lost_2v2
        end
        ...
        it "updates company total exp" do
          ...
          expect(company_stats1.reload.total_exp).to eq 142.12385
        end
      ...
      context "when the company is the loser" do
        ...
        it "updates company total exp when there are no squads" do
          ...
          expect(company_stats2.reload.total_exp).to eq 0
        end
      end
    end
